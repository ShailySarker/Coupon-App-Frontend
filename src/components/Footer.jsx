...
import { FaDiscord, FaEnvelope, FaFacebook, FaInstagram, FaYoutube } from 'react-icons/fa';
        ...
        <div>
            <div className=" bg-[#A21D3C] text-white xl:py-12 md:py-8 py-6 lg:flex lg:flex-row lg:justify-between grid md:grid-cols-2 grid-cols-1 lg:gap-0 md:gap-x-24 md:gap-y-14 gap-6 xl:px-14 lg:px-12 md:px-7 px-5">
                <div className="flex flex-col md:items-start items-center xl:w-[22%] lg:w-[25%]">
                    <h2 className="font-bold text-white xl:text-3xl md:text-2xl text-xl flex items-center gap-1"><RiCoupon5Line />Coupon App</h2>
                    <div className="xl:mt-6 md:mt-5 mt-4">
                        <p className="xl:text-base text-sm/normal font-medium"> Coupon App distributes coupons to users in a round-robin manner, ensuring fairness. It prevents abuse using IP tracking and cookie-based session tracking to restrict multiple claims within a specified time frame.</p>
                    </div>
                </div>
                <div>
                ...
                <div>
                    ...
                    <div className="flex flex-col xl:gap-3 md:gap-2 gap-[6px] xl:text-lg md:text-base text-sm xl:mt-7 md:mt-5 mt-3 font-medium">
                        <p className="flex items-center gap-2"><FaEnvelope />couponapp@gmail.com</p>
            ...
            <div className='bg-black xl:py-4 md:py-3 py-2'>
                <p className='font-medium text-[#FFFFFF] text-center xl:text-base md:text-sm text-[13px] flex items-center gap-1 justify-center'>© <span>{moment().format('YYYY')}</span><span className='font-bold flex items-center gap-[2px]'><RiCoupon5Line />Coupon App</span>. All Rights Reserved.</p>
            </div>
        </div>
    );
};
